...
<script type="text/javascript">
    var app = new Vue({
        ...
        methods: {
            ...
            querydata(url){
                axios.post(url,"begin="+1+"&num="+5)
            },
          async test(url){
                var that=this
                let data=await axios.post(url,"begin="+1+"&num="+5)
              //下面的语句后再post请求之后再执行,这个await要包裹在data这里
                console.log(data.data)
                console.log("yes")
            },
            testnow(){
                console.log("hahaha")
                let data=this.test('/jietao/queryAllTextServlet')
                //但是下面的语句不会等上面的语句执行完，他会在上面的语句执行的同时往下面执行,此时的data只是一个promise对象
                console.log(data)
                console.log("123jjjj")
                console.log("123bbbbb")
            }
        },
    })
