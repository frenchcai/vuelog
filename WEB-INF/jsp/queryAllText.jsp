...
function query(begin, num){
	...
	$.ajax({
          ...
          success:function(json){
    		...
    		"<div id=textID"+i+"><input type=hidden value="+json[i].userID" />"+"<input type=hidden value="+json[i].textID" />"+"<div class=thumbnail><img src=./images/"+json[i].username+"/"+json[i].picpath1+" /><img src=./images/"+json[i].username+"/"+json[i].picpath2+" />"+"<div class=caption><h4>"+json[i].title+"</h4><h4>"+json[i].username+"</h4><p>"+json[i].text+"</p><p><a role=button onclick=isLike("
    				+i+")>点赞</a><a role=button onclick=addToMine("+json[i].textID+")>收藏</a><a role=button onclick=sendMessage("+i+")</a></p></div></div></div>"
        		  
    		$("#shareText>div").addClass("col-sm-12 col-md-6 clo-lg-6");
    		//查找id为shareText的所有直接子代div
    		
    		
        	//	  $("#shareText").append("<tr id="+i+">"+"<td onclick=test("+i+")>"+json[i].name+"</td><td>"+json[i].sex+"</td><td>"+json[i].password+"</td><td>"+json[i].school+"</td><td>"+json[i].major+"</td><td>"+json[i].phone+"</td></tr>");
